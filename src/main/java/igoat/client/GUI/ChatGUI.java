 ...
 * <p>
 * This class implements the ActionListener interface to respond to user actions, such as clicking
 * the send button or pressing Enter.
 */
public class ChatGUI implements ActionListener {
   ...
   * <p>
   * Additionally, it sets up event listeners: - The Enter key triggers the sending of messages via
   * the connected server handler. - Window closing triggers cleanup operations such as
   * disconnecting from the server and stopping any background threads.
   * <p>
   * A new application frame is instantiated and configured with a layout that includes the above
   ...
   * <p>
   * The received messages are processed based on their type (e.g., "chat", "error", "info",
   * "whisper", "confirm") and handled accordingly by updating the chat GUI.
   * <p>
   * - For "chat" messages, the sender and content are extracted and appended to the chat area. -
   ...
   * <p>
   * If a message is improperly formatted or empty, it is ignored. The thread sleeps briefly between
   * polling operations to prevent excessive resource usage.
   * <p>
   * In case of an interruption (e.g., stopping the application), the thread terminates gracefully.
   * <p>
   * Preconditions: - The `serverHandler` instance must be properly initialized and connected to the
   * server. - The `running` flag should be true to start the thread.
   * <p>
   * Postconditions: - A thread for message reception is started and continuously processes new
   ...
   * <p>
   * This method processes the text entered in the input field (`field`) and sends it to the server
   * through the `serverHandler` if a connection is active. The message can either be a regular chat
   * message or a private message (whisper).
   * <p>
   * Functionality: 1. Retrieves the text from the input field and trims any leading or trailing
   ...
   * <p>
   * Preconditions: - `serverHandler` must be initialized and connected for messages to be sent. -
   * Assumes the input field (`field`) contains user-entered text.
   * <p>
   * Postconditions: - If the input text is valid and the server connection is active, a message is
   * sent to the server. - The input field is cleared regardless of whether a message is sent.
   * <p>
   * Notes: - Private messages must follow the specific format "/whisper <recipient> <message>".
